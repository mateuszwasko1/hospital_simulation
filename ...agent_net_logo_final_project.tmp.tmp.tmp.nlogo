...
to setup
  ...
  set counter 0
  while [counter <= amount-of-mosquitos] [
    add-mosquitos
    set counter (counter + 1)
  ]
...
to go
  update-labels ; Adds number of health to agents
...
to new-patients
  ...
  let new_arivals floor (sick_patients * 0.3)
  if length available-beds >= new_arivals[
  ]
