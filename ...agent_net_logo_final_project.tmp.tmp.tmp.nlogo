...
;    * 'exposure-threshold'  : ticks of exposure needed to become exposed

patches-own [
  contamination-level     ; how much virus is in the air on that patch (0 - 1)
]
...
turtles-own [
  exposed?               ; has been exposed and is incubating
  exposure-counter       ; ticks spent on contaminated patch
...
to setup
  ...
  set infected-by-mosquito 0
  set infected-by-air 0
  set infected-by-staff 0
...
to setup-patches
  ;; determine grid-size
  if size-of-room = "small"  [ set room-size 32 ]
  if size-of-room = "medium" [ set room-size 48 ]
  if size-of-room = "large"  [ set room-size 96 ]
  ask patches [
    set contamination-level 0
    ;; draw grid lines
    ifelse room-size > 0 and (((pxcor - min-pxcor) mod room-size) = 0 or ((pycor - min-pycor) mod room-size) = 0) [ set pcolor black ][ set pcolor white ]
  ]
end
...
to add-patient [init_health sick?]
  if length available-beds > 0 [
    ...
    create-patients 1 [
      ...
      set original-patient-list lput self original-patient-list
    ]
    set occupied-beds lput chosen occupied-beds
...
to setup-stations
  ...
  while [column < columns] [
     let row 0
     while [row < rows] [
      let x min-pxcor + (column * room-size) + 3
      let y min-pycor + (row * room-size) + 3
      create-stations 1 [
                 setxy x y
...
to go
  ...
  staff-movement ; Tells staff where to go
...
to staff-movement
  ask staffs[
    if nurse-state = "resting" and resting-counter = 0 [
  ...
  ask staffs[
    ...
    if nurse-state = "treating" [
      ...
      if exposed? and not [infected?] of current-target and not [exposed?] of current-target[
        ask current-target [person-exposed 33 "staff"]
      ]
      ...
      let time-of-treatment 0
      if size-of-room = "small" [set time-of-treatment 9]
      if size-of-room = "medium" [set time-of-treatment 6]
      if size-of-room = "large" [set time-of-treatment 3]
      if treatment-counter = size-of-room[
        set nurse-state "go-to-station"
        set current-target nobody
    ]

    ]
    ;; this deceases the working hours of the staff if they are not resting (like a countdown)
    if nurse-state != "resting"[
...
to patient-died
  let dead-patient self
      ask staffs with [ current-target = dead-patient ] [
        set current-target nobody
        set nurse-state "go-to-station"
      ]
end
...
to person-exposed [chance by_who]
  ; Agent Function
  ...
  set exposed? true
    set color yellow
    if by_who = "staff" [set infected-by-staff infected-by-staff + 1]
    if by_who = "air" [set infected-by-air infected-by-air + 1]
    if by_who = "mosquito" [set infected-by-mosquito infected-by-mosquito + 1]
  ]
end
...
to update-color-patches
  ; Updated the patches with green color coresponding to the contamination level
  ask patches [
    if pcolor != black[
...
; END OF MODEL CODE
;--------------------------------
; Buttons: 'Setup' runs setup, 'Go' runs go forever
@#$#@#$#@
GRAPHICS-WINDOW
236
10
